...
import ru.yaal.offlinewebsite.impl.job.SequenceDownloadJob;
import ru.yaal.offlinewebsite.impl.job.SequencePackageJob;
 ...
 */
@Slf4j
public class SequenceOfflineWebsite {

    public static void download(URL url, Path outletDir) {
        log.info("Start downloading: " + url);
        RootResUrl rootResUrl = new ResUrlImpl(url.toString());
        ...
        HtmlParserParams<TagNode> htmlHtmlParserParams = new HtmlParserParamsImpl<>(storage, rootResUrl, extractors, 1);
        ...
        DownloadJobParams downloadJobParams = new DownloadJobParamsImpl(rootResUrl, downloader, storage,
                null, headRetriever, parsers, headingFilters, headedFilters);
        Job downloadJob = new SequenceDownloadJob(downloadJobParams);
        downloadJob.process();

        PackageJobParams packageJobParams = new PackageJobParamsImpl(storage, packagers, null);
        Job packageJob = new SequencePackageJob(packageJobParams);
        packageJob.process();
    ...
    public static void main(String[] args) throws IOException {
        Path outletDir = Files.createTempDirectory(SequenceOfflineWebsite.class.getSimpleName());
